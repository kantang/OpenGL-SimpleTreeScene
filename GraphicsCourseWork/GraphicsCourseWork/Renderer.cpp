...
showFPS(true)
{
	...
	root = new SceneNode(heightMap,Vector4(0,0,0,1));
...
bool Renderer::GenerateTreesAndParticles()
{
	...
	for (int i = 0; i < NUMBER_OF_TREES; ++i)
	{
		...
		trees[i].SetTranslation(Vector3((float)x, 50, (float)z));
...
void Renderer::RenderScene()	{
	...
	if (showFPS)
	{
		...
		DrawText(getMemoryUsedForTrees(), Vector3(0, 16, 0), 16.0f);
		DrawText("H: More Instruction", Vector3(0, 32, 0), 16.0f);
		glEnable(GL_DEPTH_TEST);
	}
	if (showHelp)
	{
		glDisable(GL_DEPTH_TEST);
		DrawText("L: Light View", Vector3(0, 64, 0), 16.0f);
		DrawText("M: Polygon Mode", Vector3(0, 80, 0), 16.0f);
		DrawText("R: Reset Scene", Vector3(0, 96, 0), 16.0f);
		DrawText("Q: Toggle Rain / Snow", Vector3(0, 112, 0), 16.0f);
		DrawText("P: Toggle Blurring", Vector3(0, 128, 0), 16.0f);
		DrawText("O: Toggle Defered Rendering", Vector3(0, 144, 0), 16.0f);

		DrawText("Numpad8: 4 X Foward", Vector3(0, 320, 0), 16.0f);
		DrawText("Numpad6: 8 X Foward", Vector3(0, 336, 0), 16.0f);
		DrawText("Numpad2: 4 X Back", Vector3(0, 352, 0), 16.0f);
		DrawText("Numpad4: 8 X Quicker", Vector3(0, 368, 0), 16.0f);
		DrawText("Numpad5: Pause", Vector3(0, 384, 0), 16.0f);
		glEnable(GL_DEPTH_TEST);
	}
	showHelp = false;
